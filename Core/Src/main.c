...
void Charge_equalization(void){
			...
			float min = V1;          // 先假设V1是最小值
			if (V2 < min) min = V2;  // 如果V2更小，则更新最小值
			if (V3 < min) min = V3;  // 如果V3更小，则更新最小值
			if (V4 < min) min = V4;  // 如果V4更小，则更新最小值
	
			if(V1 >= BCV || V1 - min > 0.5){			DISCHARGE_CAP1_1;}			else {DISCHARGE_CAP1_0;}
			
			if(V2 >= BCV || V2 - min > 0.5){			DISCHARGE_CAP2_1;}			else {DISCHARGE_CAP2_0;}
			
			if(V3 >= BCV || V3 - min > 0.5){			DISCHARGE_CAP3_1;}			else {DISCHARGE_CAP3_0;}
			
			if(V4 >= BCV || V4 - min > 0.5){			DISCHARGE_CAP4_1;}			else {DISCHARGE_CAP4_0;}
				
			if(VV <= CCV){			CHARGE_CTRL_1;}			else {CHARGE_CTRL_0;}
...
void Power_off(void){
	...
	if (Times > 0)
	{
		...
		FLASH_ProgramHalfWord(PulseCount_START_ADDR, Total_times);	//总点焊次数
	}
//	    // 确保所有操作在电压完全下降前完成
